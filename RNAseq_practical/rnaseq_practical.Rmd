...
The [airway](http://bioconductor.org/packages/airway/) is available
from BioConductor as a data-package and contains both the gene
expression counts as well as metadata on the experiment and
...
specialized object to ease different analyses, for example, later we
will see an *DESeqDataSet* which we will construct specifically for
doing differential expression analysis using the `DESeq2`-package.
...
library(vsn)
cts <- meanSdPlot(cts, ranks = FALSE, plot=FALSE)
log.cts.one <- log2(cts + 1)
log <- meanSdPlot(log.cts.one, ranks = FALSE, plot=FALSE)
library(gridExtra)
grid.arrange(cts$gg, log$gg, ncol=2)
```
...
multiplicative factor of $2^{1.5} \aprox 2.82$.
