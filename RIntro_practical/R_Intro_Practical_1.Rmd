...
These introductory practicals are designed to teach you the basics of R. At the end, we expect you to be able to follow along with the other practicals of this course. Save your R-script or paste your answers in a document turn it in on the blackboard.  
...
As mentioned above, `dplyr` is a `tidyverse` package that is frequently used for data summary and transformation. `dplyr` code is often more intuitive to read and use that base R packages for data manipulation.
...
Here, we can quickly see the 25 individuals who are obese in our data set. A `,` can be used as a logical AND between filters. Perhaps we are interested in girls who are current smokers.
