...
![Figure](https://github.com/molepi/Molecular-Data-Science/tree/master/RIntro_practical/figure.png)
...
```{r}
fhs <- read.csv(url("https://raw.githubusercontent.com/molepi/Molecular-Data-Science/tree/master/RIntro_practical.csv"))
```
...
1. Make a for-loop, so don't use subsetting like we practiced earlier, that returns all values of MI (of data frame fhs) where BMI is greater than 30.
...
* The genomic ranges (?GRanges) object of the GenomicRanges package is often used in these packages to store genomic data (SNPs, RNA-seq, DNA methylation).
...
gr1 <- GRanges("chr1", IRanges(1:5, width = 1, names=paste0("snp", 1:5)), 
               ref = sample(c("A", "G", "C", "T"), 1), alt = sample(c("A", "G", "C", "T"), 1))
gr1
gr1[1]


gr2 <- GRanges("chr1", IRanges(8:12, width=5,
                names=paste0("gene", 1:5)), score=10:14)
