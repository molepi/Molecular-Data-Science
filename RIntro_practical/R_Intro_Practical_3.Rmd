...
Before moving on, make sure to set your library path and load in the `IRanges` package.
...
You can list the ranges with overlaps in a particular `IRanges` object after using `findOverlaps()` by utilising subsetting and `subjectHits()`.
...
So, here we see that `a`, `c`, and `d` ranges in `ir2` overlap with ranges in `ir1` once each, and `f` does not overlap with any ranges in `ir1`.
...
An important component of `GenomicRanges`is `seqinfo()`. This shows how many base pairs are in each chromosome as `seqlengths` and which genome they from.
...
We can see that the vast majority of GWAS hits lie in intergenic regions. This is the focus of a lot of research, since the regulatory mechanisms of non-coding regions is considerably complex and elusive.
