---
title: "Practical Course: Integrated analysis of multiple -omics data and Mendelian randomization"
output:
  html_document:
    toc: true
    toc_depth: 2
...
transcription (questions 1-25), Part 2 on DNA methylation is for fast
students. Paste the answers to your questions in a Word document and
mail them to b.t.heijmans@lumc.nl with your name both in the document
...
3. Use [Gene Cards](www.genecards.org) to look up the genes in
   __transcripts__, what is their reported function?
4. Use
   [USCS Genome Browser](https://genome.ucsc.edu/cgi-bin/hgGateway) to
   look up the CpGs in __cpgs__, what is their nearest gene?
...
* Use __lm__ to fit a linear model with triglycerides as explanatory
  variable and _ABCG1_ transcription as response variable and add this
  to the plot using __abline__.
...
6. Are higher triglyceride levels associated with higher or lower
   _ABCG1_ transcription levels?

* Use __summary__ on the fitted model to obtain the model
  coefficients.
...
genetic variant associated with triglyceride levels obtain from a
